...
class Day02Test : StringSpec({
    ...
    "get max count of blue cubes from game" {
        forAll(
            ...
            getMaxCountOfColorCubesFromGame(game, "blue") shouldBe maxCount
        }
    }
})

fun getMaxCountOfColorCubesFromGame(game: String, color: String): Int {
    return maxOf(
        subsets(game).map { cubes ->
            cubes.split(", ")
                .filter { colorQuantity -> colorQuantity.split(" ").last() == color }
